...
update fp_cat_fiche cf, phpbb_topics t set cf.adopte=1, cf.reserve=0, cf.parrain=0, cf.disparu=0 where cf.topic_id = t.topic_id and (t.forum_id = 11 or t.forum_id = 110 or t.forum_id = 111 or t.forum_id = 123);
...
update fp_cat_fiche fiche set fiche.date_adoption = 
 (select max(from_unixtime(log.log_time)) 
  ...
  where fiche.topic_id = log.topic_id 
        and (log.forum_id = 11 or log.forum_id = 110 or log.forum_id = 111 or log.forum_id = 123)
