...
ob_start();
$aus=Array();
$pageheader=MSDheader().headline($lang['restore']);
$aus1=$page_parameter='';
...
MSD_mysql_connect($restore['dump_encoding'],true);
//WriteLog("<br>Encoding: ".$restore['dump_encoding']);
...
else $restore['tables_to_restore']=false;
...
//WriteLog('Selbstaufruf');
	...
	$aus[]= '<br>'.sprintf($lang['restore_db'],$databases['db_actual'],$config['dbhost']).'<br>';
	$aus[]=$lang['file'].': <b>'.$restore['filename'].'</b><br>';
	$aus[]=$lang['charset'].': <strong>'.$restore['dump_encoding'].'</strong><br>';
	...
	$aus[]='<table border="0" width="100%" cellpadding="0" cellspacing="0"><tr>';
	...
	$aus[]='<tr style="padding:0;margin:0;"><td align="left" nowrap style="font-size:10px;" >'.$config['minspeed'].'</td>';
	...
	if ($restore['EOB'])
	{
		...
		$aus[]= "<br>&nbsp;&nbsp;&nbsp;<input class=\"Formbutton\" type=\"button\" value=\"".$lang['back_to_minisql']."\" onclick=\"self.location.href='".$relativ_path."sql.php'\">";
		$aus[]= "&nbsp;&nbsp;&nbsp;<input class=\"Formbutton\" type=\"button\" value=\"".$lang['back_to_overview']."\" onclick=\"self.location.href='".$relativ_path."main.php?action=db&dbid=".$databases['db_selected_index']."#dbid'\">";
		$RestoreFertig=1;
	}
	else
	{
