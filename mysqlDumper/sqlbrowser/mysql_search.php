	...
	if (sizeof($tables)>0)
	{
		...
		if (($suchbegriffe>'') && (is_array($suchworte)) )
		{
			...
			else
			{
				...
				else die('<br>Fehler bei Suche: ich konnte nicht ermitteln welche Felder die Tabelle "'.$tabelle.'" hat!');
...
if (is_array($treffer) && isset($treffer[0]))
{
	...
	for ($a=0;$a<$zeige_treffer;$a++)
	{
		...
		else $rk=build_recordkey($treffer[$a]); // no keys
		...
		foreach ($treffer[$a] as $key=>$val)
		{
			...
			$tpl->assign_block_vars(
				'HITS.TABLEROW.TABLEDATA',array('VAL' => $val));
		}
	}
}
else
