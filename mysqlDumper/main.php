...
if(isset($_POST['htaccess']) || $action=="schutz")
{
	...
	//Ausgabe
	echo headline('<img src="'.$config['files']['iconpath'].'key.gif" alt="" style="height:5px" border="0">'.$lang['htaccess1']).$Overwrite;
	if($step<2){
...
if($action=="edithtaccess") {
	...
	echo headline('<img src="'.$config['files']['iconpath'].'key.gif" alt="" border="0">&nbsp;'.$lang['htaccess16']);
...
if($action=="status") {
	...
	$status.='<img src="images/loveyourdata.gif" align="right" alt="love your data" title="lovae your data">';
	...
	switch($var) {
		...
		case "prozesse":
			...
			if($numrows==0) {echo $lang['info_noprocesses'];} else {
				echo '<table class="bdr"><tr class="thead"><th>ID</th><th>User</th><th>Host</th><th>DB</th><th>Command</th><th>Time</th><th>State</th><th>Info</th><th nowrap>RT: '.round($config['processlist_refresh']/1000).' sec</th></tr>';
				for ($i = 0; $i < $numrows; $i++) {
			    	$cl= ($i % 2) ? "dbrow" : "dbrow1";
					$row = mysql_fetch_array($res);
...
if (isset($_GET['dbid']))
{
	...
	if($numrows==0) {
		...
		echo "\n".'<br><table class="bdr" class="small"><tr class="thead"><th>Nr.</th><th>'.
		...
		for ($i = 0; $i < $numrows; $i++)
		{
			...
			{
				echo "\n".'<tr class="dbrowsel">';
				echo '<td align="right">'.($i+1).'</td><td><a href="sql.php?db='.$databases['Name'][$dbid].'&amp;tablename='.
					urlencode($row['Name']).'&amp;dbid='.$dbid.'">'.
					$row['Name'].'</a></td><td colspan="3">'.$lang['unknown_number_of_records'].'</td><td>';
