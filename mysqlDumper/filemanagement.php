...
if (isset($_POST['dump_tbl']))
{
	...
	echo '<script language="JavaScript" type="text/javascript">parent.MySQL_Dumper_content.location.href="dump.php?comment='.urlencode($dk)
		.'&sel_dump_encoding='.$dump['sel_dump_encoding'].'&config='.urlencode($config['config_file'])
		.'";</script></body></html>';
	exit;
}
...
if (isset($_POST['restore']))
{
   ...
   else $msg.= '<p class="error">'.$lang['fm_nofile'].'</p>';
}
...
switch ($action) {
	case "dump":
		...
		$confabsolute=$config['config_file'].'.conf.php';
		...
		{
			echo $databases['db_actual'].'&nbsp;&nbsp;<span>('
				.$databases['Detailinfo']['tables']." Tables, "
				.$databases['Detailinfo']['records']." Records, "
				.byte_output($databases['Detailinfo']['size']).
				')</span></strong>';
		...
		echo '<div style="display:none"><img src="'.$config['files']['iconpath'].'progressbar_dump.gif" width="120" height="12" alt=""><br><img src="'.$config['files']['iconpath'].'progressbar_speed.gif" width="120" height="12" alt=""></div>';
		...
		else
		{
			...
			else
			{
				...
				$cron_dbpraefix = $databases['praefix'][$databases['db_actual_cronindex']];
			}
		}
