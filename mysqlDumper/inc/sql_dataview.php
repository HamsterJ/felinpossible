...
echo '</p><p class="tablename">'.($tn!='' ? $lang['table'].' '.$tn.'<br>' : ''); //.'</p>'; //<p align="left">';
...
else
{
	...
	if (substr(strtoupper($sql['sql_statement']),0,8)=='SELECT *')
	{
		$sqltmp='SELECT count(*) as anzahl '.substr($sql['sql_statement'],8,strlen($sql['sql_statement'])-8);
//echo $sqltmp;
/* maybe use SQL_CALC_FOUND_ROWS? */
		$res=MSD_query($sqltmp) or die(SQLError($sql['sql_statement'],mysql_error()));
...
if($numrowsabs>0 && $Anzahl_SQLs<=1)
{
	if($showtables==0)
	{
		...
		if ($limitstart+$limitende>$numrowsabs) $command_line.=$numrowsabs; else $command_line.=$limitstart+$limitende;
...
//v($temp);
		...
		for($x=0; $x<count($temp); $x++)
		{
			...
			if($str->name==$order) $t.='<img src="'.$config['files']['iconpath'].$arname.'" alt="" border="0" align="left">';
...
//echo "<br><br>Primaerschluessel erkannt: ".$primary_key;
			...
			foreach ($sortkey as $rowkey=>$rowval)
			{
				...
				if($erste_spalte==1)
				{
					...
					if($key==-1)
					{
						$p.='&amp;recordkey='.build_recordkey($temp[0]);
					}
					else
					{
						//Key vorhanden
					...
					else
					{
						if($tabellenansicht==1 && $showtables==1)
						{
							...
							$d.='<a href="'.$p.'&amp;mode=empty" onclick="if(!confirm(\''.sprintf($lang['asktableempty'],$tablename).'\')) return false;">'.$icon['empty'].'</a>&nbsp;'.$nl.$nl;
							$d.='<a href="'.$p.'&amp;mode=emptyk" onclick="if(!confirm(\''.sprintf($lang['asktableemptykeys'],$tablename).'\')) return false;">'.$icon['empty_keys'].'</a>&nbsp;'.$nl.$nl;
				...
				else
				{
					if ($showtables==0)
					{
						if (isset($fdesc[$rowkey]['type']))
							 $data=($fdesc[$rowkey]['type']=='string' || $fdesc[$rowkey]['type']=='blob') ? convert_to_utf8(strip_tags($rowval)) : $rowval;
					}
					else
					{
						if (isset($temp[$i][$rowkey]))
							 $data=($fdesc[$rowkey]['type']=='string' || $fdesc[$rowkey]['type']=='blob') ? convert_to_utf8(strip_tags($temp[$i][$rowkey])) : $temp[$i][$rowkey];
						else $data='';
