...
//echo "<br><br>Zeile: ".$zeile;
		...
		IF ($sqlparser_status==0)
		{
			...
			if(substr($zeile2,0,11)=='LOCK TABLES') $sqlparser_status=4;
			if(substr($zeile2,0,6)=='COMMIT') $sqlparser_status=7;
			if(substr($zeile2,0,5)=='BEGIN') $sqlparser_status=7;
			if(substr($zeile2,0,13)=='UNLOCK TABLES') $sqlparser_status=4;
			if(substr($zeile2,0,4)=='SET ') $sqlparser_status=4;
			if(substr($zeile2,0,6)=='START ') $sqlparser_status=4;
			if(substr($zeile2,0,3)=='/*!') $sqlparser_status=5; //MySQL-Condition oder Kommentar
            if(substr($zeile2,0,11)=='ALTER TABLE') $sqlparser_status=4;
		...
		if($sqlparser_status==2)
		{
			...
			if($last_char==';' )
			{
					...
					if (is_array($restore['tables_to_restore']))
					{
						...
						{
							$restore['actual_fieldcount']=submit_create_action($complete_sql,true); // Tabelle temporaer anlegen
		...
		if($sqlparser_status==3)
		{
			...
			if(!isset($restore['num_table_fields'][$restore['actual_table']]))
			{
				// Seitenaufruf fand mitten in der Tabelle statt - Wert aus $_POST holen
				$restore['num_table_fields'][$restore['actual_table']]=$restore['actual_fieldcount'];
			}
...
//echo "<br>Parser Status: ".$sqlparser_status."<br>SQL: ".$complete_sql;
...
function submit_create_action($sql,$temporary=false)
{
	//Führt eine Create-Anweisung durch und ermittelt danach die Anzahl der aktuellen Spalten
	// und legt diese im Array ab (Wert fuer das Parsen benoetigt)
	global $restore;
	...
	if ($temporary==true)
	{
		// Tabellenname auf einen temporaeren Namen verbiegen,
		// um nach der Erstellung die Anzahl der Felder zu ermitteln | wird zum Parsen benoetigt
		$sql=str_replace($tablename,'msd_temporary_18432',$sql);
	}
	...
	if (!$res===false)
	{
		if ($temporary===false) $restore['table_ready']++;
	}
	else
	{
	...
	if ($temporary) $anzahl_felder=get_num_rows('msd_temporary_18432');
	else $anzahl_felder=get_num_rows($tablename);

	// temporaere Tabelle wieder loeschen
	if ($temporary)
	{
		$sql='DROP TABLE `msd_temporary_18432`';
		$res=mysql_query($sql);
	}
...
{
	$t_start=13;
	$c_start=substr($t,$t_start,1);
	// 2 faelle 1. Startzeichen ist kein Backtick -> naechstes Leerzeichen finden
	$t_end=14;$found=false;
	if ($c_start!='`')
	{
		WHILE (!$found)
		{
			if (substr($t,$t_end,1)==' ') $found=true;
			if ($t_end==strlen($t)) $found=true;
			$t_end++;
		}
		$t_end--;
	}
	...
	{
		// zweitens Startzeichen ist ein Backtick  -> dann bis zum naechsten Backtick suchen
		$t_start++;
		WHILE (!$found)
		{
			if (substr($t,$t_end,1)=='`') $found=true;
	...
	$tn=substr($t,$t_start,$t_end-$t_start);
	return trim($tn);
}
