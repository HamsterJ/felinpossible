...
function getCountSQLStatements($sql) {
    ...
    for($i=0;$i<$l;$i++) {
        ...
        if($sql[$i]==';' && $inQuotes==false) $z++;
    }
    return $z;
}

function splitSQLStatements2Array($sql) {
    $z=0;$sqlArr=array();$tmp='';
    $sql=str_replace("\n",'',$sql);
    $l=strlen($sql);
    $inQuotes=false;
    for($i=0;$i<$l;$i++) {
...
function SQLParser($command,$debug=0)
{
	...
	if(substr($s,0,1)=="#" || substr($s,0,2)=="--") {
		...
		if(strtoupper(substr($s,0,7))=="INSERT ") $sql['parser']['insert']++; else $sql['parser']['update']++;
...
//	if($meldung!="") $s.='<p class="success">'.$meldung.'</p>';
	...
	if(count($SQL_ARRAY)==0) {
		...
		if(count($SQL_ARRAY)>0) {
			for($i=0;$i<count($SQL_ARRAY);$i++) {
		        ...
		        $s=str_replace('"','&quot;',SQL_String($i));
				$csql=stripslashes(str_replace('"','&quot;',$csql));
...
function ExtractTablenameFromSQL($q)
{
	...
	switch($first)
	{
		case "SHOW":
			...
			break;
		case "SELECT":
			$p=substr($q,strpos($p,"FROM")+5);
			$p=explode(" ",$p);
			$t=$p[0];
			if(strpos($t,".")) $t=substr($t, strpos($t,".")+1);
			$table=str_replace("`","",$t);
			break;
		case "DROP":
		case "INSERT":
	...
	// So, dann wollen wir doch mal nachsehen, ob es die ermittelte Tabelle ueberhaupt gibt
	if (Table_Exists($databases['db_actual'],$table)) return $table;
	else return "";
}
