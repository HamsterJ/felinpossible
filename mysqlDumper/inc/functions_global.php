...
/*
v($parameter);
echo htmlspecialchars($page_parameter);
WriteLog(htmlspecialchars($page_parameter));
die();
*/
...
function AutoDelete()
{
	...
	// Backups von Fremdprogrammen bleiben unberuecksichtigt!
	...
	while (false !== ($filename = readdir($dh)))
	{
	    if ($filename != "." && $filename != ".." && !is_dir($config['paths']['backup'].$filename))
	    {
			...
			$file_db_name=$statusline["dbname"];
			$datum=substr($filename,strlen($file_db_name)+1);
			$datum=substr($datum,0,16);

			// Backups pro DB zaehlen
			if(isset($dbbackups[$file_db_name])) $dbbackups[$file_db_name]++;
			else $dbbackups[$file_db_name]=1;
			if ($part>0)
			{
				// Multipartfiles nur einmal eintragen - in Part steht der letzte Part
				// so weiss man, welche Dateien zu loeschen sind
				...
				if (is_array($files) && sizeof($files)>0)
				{
					...
					{
						$d=explode("|",$files[$i]);
						if ($d[0]==$datum && $d[1]==$file_db_name && $d[2]<$part)
						{
							// Eintrag mit hoeherer Part-Nr ersetzen
							$files[$i]=$datum."|".$file_db_name."|".$part."|".$filename;
							$eintrag_vorhanden=true;
						}
						unset($d);
					}
				}
				if (!$eintrag_vorhanden) $files[]=$datum."|".$file_db_name."|".$part."|".$filename;
			}
			else
			{
				if ($statusline["dbname"]!="unknown") $files[]=$datum."|".$file_db_name."|".$part."|".$filename;
			}
		}
	}
...
function TesteFTP($i)
{
	...
	if(!extension_loaded("ftp")) {
		$s='<br><span class="error">'.$lang['noftppossible'].'</span>';
	} else $pass=0;
	...
	if($pass==0)
	{
		...
		{
			$s='<br><span class="error">'.$lang['wrongconnectionpars'].'</span>';
		} else $pass=1;
	}

	if($pass==1)
	{
		$s=$lang['connect_to'].' `'.$config['ftp_server'][$i].'` Port '.$config['ftp_port'][$i];
		...
		if(!$conn_id || (!$login_result)) {
			$s.='<br><span class="error">'.$lang['conn_not_possible'].'</span>';
		}
		else
		{
			$pass=2;
	...
	if($pass==2)
	{
		$s.='<br><strong>Login ok</strong><br>'.$lang['changedir'].' `'.$config['ftp_dir'][$i].'` ';
		...
		{
			$pass=3;
			$s.='<span class="success">'.$lang['ok'].'</span>';
		}
		@ftp_close($conn_id);
	}

	if($pass==3) $s.='<br><strong>'.$lang['ftp_ok'].'</strong>';
	return $s;
}
...
function GetThemes() {
	...
	while (false !== ($filename = readdir($dh)))
	{
	    if ($filename != "." && $filename != ".." && is_dir($config['paths']['root']."css/".$filename)
	    	&& substr($filename,0,1)!='.' && substr($filename,0,1)!='_')
	    {
...
function GetLanguageCombo($k="op",$class="",$name="",$start="",$end="") {
	...
	while (false !== ($filename = readdir($dh)))
	{
	    ...
	    {
			if (isset($lang[$filename])) $lang_files[$lang[$filename]]=$filename;
		}
	}
	ksort($lang_files);
...
function headline($title,$mainframe=1) {
	...
	if($config['interface_server_caption']==1) {
		if($config['interface_server_caption_position']==$mainframe) {
			$s.='<div id="server'.$mainframe.'">'.$lang['server'].': <a class="server" href="'.getServerProtocol().$_SERVER['SERVER_NAME'].'" target="_blank" title="'.$_SERVER['SERVER_NAME'].'">'.$_SERVER['SERVER_NAME'].'</a></div>';
		}
	}
	if ($mainframe==1) {
...
function MSDHeader($kind=0,$onloadFunction="")
{
	...
	$r.="</head>\n<body".(($kind==1) ? '' : ' class="content"').(($onloadFunction!="") ? ' onload="'.$onloadFunction.'();"' : '').'>';
...
function MSDFooter($rfoot="",$enddiv=1)
{
	...
	{
		$f= '<p id="footer">'.$lang['authors'].': <a href="http://www.mysqldumper.de" target="_blank">
...
function read_config($file=false)
{
	...
	{
		include($config['paths']['config'].$file.'.php');
...
function get_config_filenames()
{
	...
	while (false !== ($filename = readdir($dh)))
	{
	    if (substr($filename,-4)=='.php' && substr($filename,-9)!='.conf.php')
	    {
...
function table_output($text,$val,$small=false,$colspan=1)
{
	...
	else $ret.='<strong>'.$val.'</strong></td></tr>';
	return $ret;
}

// get all possible MySQL character sets and save the MySQL-Servers`s standard character_set in $config['mysql_standard_charset']
