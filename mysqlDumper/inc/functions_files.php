...
function FileList($multi=0)
{
	...
	$fl.='<table class="bdr">';
	$fl.='<tr><td colspan="8" align="left"><strong>'.$Theader.'</strong></td><td colspan="3" align="right"><a href="filemanagement.php?action='.$action.'&amp;kind='.$akind.'" class="small">'.$Wheader.'</a></td></tr>';
	...
	if($arrayindex>0) {
		for($i=$arrayindex;$i>=0;$i--) {
			if(isset($db_backups[$i]['db']) && $db_backups[$i]['db']==$dbactiv)
			{
				...
				$fl.='>';
				$fl.='<td align="left" colspan="2" nowrap>';
				if($multi==0)
				{
				...
				$fl.='<td class="sm" nowrap>'.$db_backups[$i]['date'].'</td>';
				...
				$fl.='</td><td  style="text-align:right;padding-right:12px;" nowrap>';
	...
	if(count($db_summary_anzahl)>0)
	{
		...
		while(list($key, $val) = each($db_summary_anzahl))
		{
			...
			$fl.='<td class="sm" nowrap>'.((isset($db_summary_last[$key])) ? $db_summary_last[$key] : "").'</td>';
...
function Converter($filesource,$filedestination,$cp) {
	...
	WHILE (!$eof)
	{
		...
		if (substr($zeile,0,2)=='--' || substr($zeile,0,1)=='#') $zeile='';
		$zeile=rtrim($zeile);
