...
if (isset($_GET['config_delete']))
{
	...
	else $msg='<p class="success">'.sprintf($lang['success_deleting_configfile'],$del_config).'</p>'
			.'<script type="text/javascript">parent.MySQL_Dumper_menu.location.href="menu.php?config='.$config['config_file'].'";</script>'; //refresh menu-frame
	$sel='configs';
}
...
if (isset($_POST['save']))
{
	...
	$databases['db_actual_cronpraefix']=isset($_POST['dbcronpraefix']) ? $_POST['dbcronpraefix']:'';
	...
	$config['maxspeed']=$_POST['maxspeed']; if($config['maxspeed']<$config['minspeed']) $config['maxspeed']=$config['minspeed']*2;
...
//$config['cron_ftp']=$_POST['cron_ftp'];
	...
	$databases['db_actual_cronindex']=isset($_POST['cron_dbindex']) ? $_POST['cron_dbindex']:0;
...
$aus['conf'].='<td><input class="Formbutton" type="submit" name="save" value="'.$lang['save'].'"></td>';
$aus['conf'].='</tr></table>';
...
if (sizeof($configs)>0)
{
	foreach ($configs as $c)
	{
		...
		$aus['conf'].='<img src="css/'.$old_config['theme'].'/icons/search.gif" align="right" alt="" title=""><strong>'.sizeof($databases['Name']).'</strong></a>';
		$aus['conf'].='</td></tr>';
		...
		else
		{
			...
			if ($databases['praefix'][$databases['db_selected_index']]>'') $text.=" ('<i>".$databases['praefix'][$databases['db_selected_index']]."</i>')";
		...
		{
			$text=$databases['Name'][$databases['db_actual_cronindex']];
			if ($databases['praefix'][$databases['db_actual_cronindex']]>'') $text.=" ('<i>".$databases['praefix'][$databases['db_actual_cronindex']]."</i>')";
			$aus['conf'].=table_output($lang['backup_dbs_perl'], $text);
		}
...
$aus['global1'].='<input type="text" class="text" id="multipartgroesse1" name="multipartgroesse1" size="3" maxlength="3" value="'.$config['multipartgroesse1'].'"';
...
//$aus['cron'].='<td><table><tr><td>'.$lang['existing'].':</td><td><select name="cron_select_savepath">'.get_config_filelist().'</select></td></tr>';
//$aus['cron'].='<tr><td>'.$lang['new'].':</td><td><input class="text" type="text" size="18" name="cron_savepath_new" value=""></td></tr></table></td>';
...
$aus['cron'].='<td><input type="radio" class="radio" value="1" name="cron_samedb" onclick="obj_disable(\'cron_dbindex\');obj_disable(\'dbcronpraefix\');" '.(($config['cron_samedb']==1)?" checked":"").'>&nbsp;'.$lang['yes'];
$aus['cron'].='&nbsp;&nbsp;&nbsp;<input type="radio" class="radio" value="0" name="cron_samedb" onclick="obj_enable(\'cron_dbindex\');obj_enable(\'dbcronpraefix\');" '.(($config['cron_samedb']==0)?" checked":"").'>&nbsp;'.$lang['no'];
...
if ($config['cron_samedb']==1) $aus['cron'].='disabled';
$aus['cron'].='>';
if (isset($databases['Name'][0]) && $databases['Name'][0]>'')
{
	$datenbanken=count($databases['Name']);
...
$aus['cron'].='</select>&nbsp;&nbsp;<input type="text" class="text" name="dbcronpraefix" id="dbcronpraefix" value="'.$databases['db_actual_cronpraefix'].'"';
if ($config['cron_samedb']==1) $aus['cron'].='disabled';
$aus['cron'].='></td></tr>';
